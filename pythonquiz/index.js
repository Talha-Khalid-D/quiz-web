...
const loadquiz = () =>{
    const {Question , Options} = pyquizData[currentquiz];
    // console.log(Options);
...
submitbtn.addEventListener('click', ()=>{
    ...
    if(selectedOptionIndex === pyquizData[currentquiz].Correct){
        score = score + 1;
    }
    ...
    if(currentquiz < pyquizData.length){
        deselectedAnswers();
        loadquiz();
    }else{
        quiz.innerHTML = `
        <div class="result">
        <h2> Your Score : ${score} / ${pyquizData.length} Correct Answers </h2>

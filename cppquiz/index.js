...
const loadquiz = () =>{
    const {Question , Options} = cpquizData[currentquiz];
    // console.log(Options);
...
submitbtn.addEventListener('click', ()=>{
    ...
    if(selectedOptionIndex === cpquizData[currentquiz].Correct){
        score = score + 1;
    }
    ...
    if(currentquiz < cpquizData.length){
        deselectedAnswers();
        loadquiz();
    }else{
        quiz.innerHTML = `
        <div class="result">
        <h2> Your Score : ${score} / ${cpquizData.length} Correct Answers </h2>
